...
echo "# CP540" >> README.md
git init
git add README.md
git commit -m "first commit"
git branch -M main
git remote add origin https://github.com/ZamzamJ99/CP540.git
...
def assign2(url):
    ...
    for month in months: #iterate for all months
        ...
        meanRads.append(np.mean(sumDaily))   #mean of each day in a month, taking average of every month
  
    maxMeanRads = max(meanRads) # max monthly radiation in given year
    
    H = 24; #time in hours
    t = 60*60*H; # time in seconds
    
    avgRads = np.array(meanRads) #convert to array
    avgRadsAct = (avgRads *0.15)/t #calculate actual avg radiation based on given accuracy
    maxRadsAct = (maxMeanRads*0.15)/t #calculate max avg monthly radiation based on given accuracy
    
    
    a_t = 1*365*24; #convert year to hours
    h_e = 5000/a_t; # convert annual consumption to from kWh to kW
    
    a = h_e / avgRadsAct; #calculate area for each month in a given year
    a1 = np.mean(a); #take average to find avg area for year
    minA = h_e / maxRadsAct;#calculate minimum area for year
    return meanRads, a1, minA

#utilise function for 3 most recent years
meanRads2021, area2021,minA2021 = assign2('midas-open_uk-radiation-obs_dv-202207_greater-london_00708_heathrow_qcv-1_2021.csv')
meanRads2020, area2020,minA2020 = assign2('midas-open_uk-radiation-obs_dv-202107_greater-london_00708_heathrow_qcv-1_2020.csv')
meanRads2019, area2019,minA2019 = assign2('midas-open_uk-radiation-obs_dv-202207_greater-london_00708_heathrow_qcv-1_2019.csv')
